...
namespace ElasticsearchNEST.Data.Repositories
{
    public class Repository : Domain.Interfaces.IRepository
    {
        private readonly IElasticClient _elasticClient;
        private readonly string _indexName = nameof(IndexItem).ToLower();
        ...
        public Repository(IElasticClient elasticClient)
        {
            ...
            if (_elasticClient.Indices.Exists(_indexName).Exists is false)
        ...
        public ICollection<IndexItem> FindAll()
        {
            return _elasticClient.Search<IndexItem>(x =>
                x.Index(_indexName)
                    .From(0)
                    .Size(30)
        ...
        {
            var query = new QueryContainerDescriptor<IndexItem>().Term(t => t.Field(f => f.Name).Value(name));
            ...
            return _elasticClient.Search<IndexItem>(x =>
                x.Index(_indexName)
                    ...
                    .From(0)
                    .Size(30)
                    .Sort(q => q.Descending(p => p.BirthDate)))?.Documents?.ToList();
        }

        public ICollection<IndexItem> FindByDescription(string description)
        {
            var query = new QueryContainerDescriptor<IndexItem>().Match(t => t.Field(f => f.Description).Query(description)); //case insensitive, OU
            
            var result = _elasticClient.Search<IndexItem>(s => s
                .Index(_indexName)
                .Query(x => query)
                .From(0)
                .Size(30)
        ...
        public ICollection<IndexItem> FindActorsCondition(string name, string description, DateTime? birthdate)
        {
            throw new NotImplementedException();
        }

        public ICollection<IndexItem> FindActorsAllCondition(string term)
        {
            throw new NotImplementedException();
        }

        public static List<IndexItem> GetSampleData()
        {
            var list = new List<IndexItem>
            {
                new IndexItem
                {
                    Id = "38d61273-8f4e-431c-a0bb-3de2fbbf8757", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1969, 9, 25), Age = 50, TotalMovies = 25, Name = "Catherine Zeta-Jones",
                    Description =
                ...
                new IndexItem
                {
                    Id = "6d288857-34e4-417f-8a22-e0c7575972b5", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1948, 12, 27), Age = 71, TotalMovies = 44, Name = "Gérard Depardieu",
                    Description =
                ...
                new IndexItem
                {
                    Id = "9dc521a4-3486-4c6c-b769-8bdafcc4563d", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1976, 1, 13), Age = 44, TotalMovies = 34, Name = "Michael Peña",
                    Description =
                        "Peña nasceu em Chicago, Illinois, onde seu pai trabalhava em uma fábrica de botões e sua mãe era uma assistente social.[1][2] Os pais de Peña, imigrantes provenientes do México, foram originalmente agricultores.[3][4] Peña frequentou a Hubbard High School em Chicago.[5] Peña e sua esposa, Brie Shaffer, tiveram seu primeiro filho em setembro de 2008, um menino chamado Roman.[6]"
                },
                new IndexItem
                {
                    Id = "b9044bf7-6917-4521-9b1b-f737a05965f9", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1944, 9, 25), Age = 75, TotalMovies = 52, Name = "Michael Douglas",
                    Description =
                ...
                new IndexItem
                {
                    Id = "abc6caf0-6c60-4e9e-93d2-2f53fbe941e2", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1956, 7, 9), Age = 63, TotalMovies = 41, Name = "Tom Hanks",
                    Description =
                        "Thomas Jeffrey Hanks, mais conhecido como Tom Hanks[1] (Concord, 9 de julho de 1956),[2] é um premiado ator, produtor, dublador, roteirista e diretor norte-americano. Destacou-se em diversos filmes de sucesso, como: Forrest Gump, Apollo 13, That Thing You Do!, The Green Mile, The Terminal, Inferno, Saving Private Ryan, You've Got Mail, Sleepless In Seattle. Charlie Wilson's War, Catch Me If You Can, Cast Away, A League Of Their Own, The Da Vinci Code, Captain Phillips, Angels & Demons, Splash, Big, Road To Perdition, Philadelphia e como a voz do personagem Woody na série de filmes de animação Toy Story e também pelas vozes em The Polar Express."
                },
                new IndexItem
                {
                    Id = "7b0636fd-42ca-4435-a9ff-bebef6a40fda", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1933, 3, 14), Age = 87, TotalMovies = 69, Name = "Michael Caine",
                    Description =
                ...
                new IndexItem
                {
                    Id = "b7666135-d20b-4a52-98f2-87023d4c2bdb", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1974, 1, 30), Age = 46, TotalMovies = 42, Name = "Christian Bale",
                    Description =
                        "Christian Bale, para cada dez atores mirins que, quando adultos, não conseguem continuar sua carreira, existe um Christian Bale. O menino escolhido pessoalmente por Steven Spielberg para estrelar Império do Sol cresceu e se tornou um dos talentos mais respeitados e um dos homens mais desejados de Hollywood.[carece de fontes]"
                },
                new IndexItem
                {
                    Id = "6b0b2f52-ee2d-4b90-9dcc-c303d1b9830c", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1943, 8, 17), Age = 76, TotalMovies = 56, Name = "Robert De Niro",
                    Description =
                        "O primeiro filme em que Robert De Niro participou foi a produção francesa de 1965 Trois Chambres à Manhattan, de Marcel Carné. Niro fazia um papel secundário, como cliente de um restaurante.[4] Depois participou em muitos outros filmes, com papéis maiores e menores, mas nenhum deles com grande sucesso, até que atingiu a popularidade com o seu papel em Bang the Drum Slowly (br: A Última Batalha de um Jogador), em 1973. "
                },
                new IndexItem
                {
                    Id = "2eda34e3-3cdb-47b7-ac56-c5946d7d695a", RegistrationDate = DateTime.Now,
                    BirthDate = new DateTime(1940, 4, 25), Age = 80, TotalMovies = 78, Name = "Al Pacino",
                    Description =
